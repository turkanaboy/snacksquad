...
function BracketHistory({ archive }: { archive: BracketArchive[] }) {
  return <section className="bracket-history" aria-labelledby="bracket-history-title"><div className="section-heading"><div><h2 id="bracket-history-title">Past bracket winners</h2><p>Completed weekly podiums, including shared third place for both semifinalists.</p></div><span>{archive.length}</span></div><div className="bracket-history-list">{archive.map((result) => <details key={result.weekId}><summary><span>Week of {weekLabel(result.weekStart)}</span><b>{result.firstPlace.snackName}</b></summary><ol><li><span>1st</span><b>{result.firstPlace.snackName}</b></li>{result.secondPlace ? <li><span>2nd</span><b>{result.secondPlace.snackName}</b></li> : null}{result.thirdPlace.length ? <li><span>3rd (tie)</span><b>{result.thirdPlace.map((entry) => entry.snackName).join(" · ")}</b></li> : null}</ol></details>)}</div></section>;
}
