...
export type FantasyOverview = {
  ...
  season: null | { id: string; month: string; status: string; currentPick: number; pickDeadline: string | null; scoringEndsAt: string };
...
export async function getFantasyOverview(client: RpcClient, leagueId: string): Promise<FantasyOverview> {
  ...
  return { league:{id:raw.league.id,name:raw.league.name,joinCode:raw.league.join_code}, members:(raw.members||[]).map((x:{user_id:string;display_name:string})=>({userId:x.user_id,displayName:x.display_name})), season:raw.season?{id:raw.season.id,month:raw.season.month,status:raw.season.status,currentPick:Number(raw.season.current_pick),pickDeadline:raw.season.pick_deadline,scoringEndsAt:raw.season.scoring_ends_at}:null, draftOrder:(raw.draftOrder||[]).map((x:{user_id:string;position:number})=>({userId:x.user_id,position:Number(x.position)})), picks:(raw.picks||[]).map((x:{user_id:string;snack_id:string;snack_name:string;category:string;pick_number:number;was_auto_pick:boolean})=>({userId:x.user_id,snackId:x.snack_id,snackName:x.snack_name,category:x.category,pickNumber:Number(x.pick_number),wasAutoPick:x.was_auto_pick})), roster:(raw.roster||[]).map((x:{user_id:string;snack_id:string;snack_name:string;category:string})=>({userId:x.user_id,snackId:x.snack_id,snackName:x.snack_name,category:x.category})), standings:(raw.standings||[]).map((x:{user_id:string;points:number})=>({userId:x.user_id,points:Number(x.points)})) };
}
...
export const startFantasyDraft=(client:RpcClient,leagueId:string,month:string)=>rpcVoid(client,"start_fantasy_draft",{p_league_id:leagueId,p_month:month}) as Promise<string>;
...
export const submitFantasyWaiver=(client:RpcClient,seasonId:string,outgoingId:string,incomingId:string)=>rpcVoid(client,"submit_fantasy_waiver",{p_season_id:seasonId,p_outgoing_snack_id:outgoingId,p_incoming_snack_id:incomingId}) as Promise<void>;
